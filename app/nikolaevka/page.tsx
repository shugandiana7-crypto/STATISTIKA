...
export default function NikolaevkaPage() {
  ...
  const mainSheet = data?.sheets.find(s => s.name === "ТО Июнь 2026 Николаевка");
    ...
    <div className="min-h-screen bg-[#0b1120]">
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6 lg:p-9">
          <h1 className="text-3xl font-bold text-white mb-6">ТО НИКОЛАЕВКА - ИЮНЬ 2026</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            ...
            <Link href="/nikolaevka/artikuly" className="bg-gradient-to-br from-purple-600 to-purple-800 p-6 rounded-2xl hover:scale-105 transition-transform">
              <h3 className="text-white font-bold text-lg">Артикулы по моделям</h3>
              <p className="text-purple-200">{data?.sheets.find(s => s.name === "Артикулы по моделям")?.data.rows.length || 0} записей</p>
            </Link>
          </div>
          ...
          {mainSheet && (
            <DataTable 
              ...
              title="ПОТРЕБНОСТЬ В ФИЛЬТРАХ И СМАЗОЧНЫХ МАТЕРИАЛАХ - НИКОЛАЕВКА" 
            />
          )}
        </main>
      </div>
